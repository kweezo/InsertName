...
i_StaticInstanceData::i_StaticInstanceData(ModelHandle model, std::weak_ptr<i_Shader> shader): model(std::move(model)),
...
void i_StaticInstanceData::AssignCommandBuffers(const std::array<i_CommandBuffer, MAX_FRAMES_IN_FLIGHT> &commandBuffers) {
...
void i_StaticInstanceData::RecordCommandBuffer(uint32_t threadIndex) {
    ...
    vkCmdBindVertexBuffers(commandBuffers[i_Swapchain::GetFrameInFlight()].GetCommandBuffer(), 1, 1, &vertexBuffer, &offset);
