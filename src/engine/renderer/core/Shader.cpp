...
namespace renderer {
    ...
    void i_ShaderManager::Init() {
        ...
        for (const Json::Value &shader: shaders) {
            ...
            std::shared_ptr<i_Shader> shaderPtr = std::make_shared<i_Shader>(
                vertexPath, fragmentPath, name, descriptorBindings, inputDescriptions);
    ...
    void i_Shader::CreateDescriptorSets() {
        if (shaderBindings.empty()) {
            std::cout << "Warning, tried to enable new shader bindings but no new shaders have been created" <<
                    std::endl;
            return;
        }
    ...
    void i_Shader::UpdateDescriptorSet(std::vector<VkWriteDescriptorSet> writeDescriptorSets) {
        vkUpdateDescriptorSets(i_Device::GetDevice(), writeDescriptorSets.size(), writeDescriptorSets.data(), 0,
                               nullptr);
    }
